...
//Loop over all events, data written to the root tree
{
    ...
    else
    {
        ...
        cout<<"file Hit finder "<<fileHitFinderPar<<endl;
            ...
            if(M2==1 && M3==1) {
                ...
                for(Int_t j=0;j<nrpchits;j++) {
                    ...
                    if(rpchit->getTrbnum()==2) {
                        dt_t3   = rpchit->getTime();
                        dt_x3   = rpchit->getX();
                        dt_y3   = rpchit->getY();
                        dt_z3   = rpchit->getZ();
                        dt_col3 = rpchit->getCol()-1;
                        dt_row3 = rpchit->getRow()-1;
                ...
                h_dt_2 -> Fill(dt_t3 - dt_t2 - sqrt( pow((dt_x3-dt_x2),2)
                                                    +  pow((dt_y3-dt_y2),2)
                                                    +  pow((dt_z3-dt_z2),2))/299.792458) ;

                h_dt_cell_2 -> Fill(dt_col3*10+dt_row3,dt_t3 - dt_t2 - sqrt( pow((dt_x3-dt_x2),2)
                                                                          +  pow((dt_y3-dt_y2),2)
                                                                          +  pow((dt_z3-dt_z2),2))/299.792458);
...
void doStaffCalibration(char* path,char* name) {
    ...
    TString day(fName(fName.First(".")-12,fName.First(".")-5));
    cout<<"DAY "<<day<<endl;
    ...
    unpack.setFileLookupPar("luptab_20160722.txt");
...
//    unpack.setFileHitFinderPar("../specialDST/pars/"+day+"_CalPars.txt");
    ...
    // Tracking Teest Line
    unpack.setFileHitFinderParOut("../2016DST/pars/"+day+"_CalPars.txt");
    unpack.fillCalibration(fPath,fName,"../2016DST/qcalhistos/", day+"_qhistos.root",1000000,0);
...
void doStaffAnalysis(char* path,char* name){
    ...
    TString fPath(path);
    Filler fill =  Filler();
    //TString day(fName(14,fName.First(".")-14));
    TString day(fName(fName.First(".")-12,fName.First(".")-5));
    ...
    fill.setFileLookupPar("luptab_20160722.txt");
    ...
    // Tracking Test Lines
    fill.setFileHitFinderPar("../2016DST/pars/"+day+"_CalPars.txt");
    fill.fillHistograms(fPath,fName,"../2016DST/results/", day+"_result_histos.root",1000000,0);
